...
export type IConfig = Partial<typeof config>;

export const getConfig = (useCache = true, cfg?: IConfig) => {
  if (useCache && config.configPath) return config;

  if (!config.configPath) config.configPath = CONF_FILE_NAME;
  if (cfg) {
